...
| 5 | `/src` com arquivos `.cs` contendo `app.MapGet` ou `app.MapPost` | ✅ 1 | `src/Backend/Program.cs` registra mais de 20 rotas com `app.MapGet`, `app.MapPost`, `app.MapPut` e `app.MapDelete` (ex: `/api/eventos`, `/api/cupons`, `/api/reservas`, `/api/auth/login`, `/api/me`). |
